...
const MAGASINS_DEFAUT=["Casino Mandarine Golf","Casino Mandarine Angré","Casino Mandarine Riviera 4","Casino Mandarine Bietry","Casino Mandarine Danga","Casino Mandarine Marcory","Casino Mandarine Koumassi","Casino Mandarine Ambassade","Casino Mandarine Monop"];
const DB_KEY="mandarine-audits";
const MAGASINS_KEY="mandarine-magasins";
const DRAFTS_KEY="mandarine-drafts";
...
function daysDiff(d){ if(!d)return null; const n=new Date();n.setHours(0,0,0,0);return Math.round((new Date(d)-n)/86400000); }
...
// ── Storage ────────────────────────────────────────────────────────────
async function dbLoad(){ try{ const r=await window.storage.get(DB_KEY); return r?JSON.parse(r.value):[]; }catch(e){return[];} }
async function dbSave(a){ try{ await window.storage.set(DB_KEY,JSON.stringify(a)); }catch(e){} }
async function draftsLoad(){ try{ const r=await window.storage.get(DRAFTS_KEY); return r?JSON.parse(r.value):[]; }catch(e){return[];} }
async function draftsSave(a){ try{ await window.storage.set(DRAFTS_KEY,JSON.stringify(a)); }catch(e){} }
async function draftDelete(id){ try{ const a=await draftsLoad(); await draftsSave(a.filter(d=>d.id!==id)); }catch(e){} }
async function magasinsLoad(){
  try{ const r=await window.storage.get(MAGASINS_KEY); if(r){return[...new Set([...MAGASINS_DEFAUT,...JSON.parse(r.value)])];} return[...MAGASINS_DEFAUT]; }catch(e){return[...MAGASINS_DEFAUT];}
}
async function magasinsSave(a){ try{ await window.storage.set(MAGASINS_KEY,JSON.stringify(a)); }catch(e){} }

// ── CSV ────────────────────────────────────────────────────────────────
function csvRow(r){ return r.map(c=>'"'+(c||"").toString().replace(/"/g,'""')+'"').join(","); }
function toCSVSingle(audit){
  const zl=ZONES.map(z=>({...z,items:[...z.items,...((audit.customItems||{})[z.id]||[])]}));
  ...
  const meta=[["RAPPORT AUDIT SUPERVISEUR - RESEAU MANDARINE"],["Date",audit.date],["Magasin",audit.magasin],["Superviseur",audit.superviseur],["Score",scoreOf(audit.results)+"%"],[]];
...
function toCSVDetail(list,filtre){
  ...
  const rows=[];
  l.forEach(a=>{
    const zl=ZONES.map(z=>({...z,items:[...z.items,...((a.customItems||{})[z.id]||[])]}));
...
// ── Atoms ──────────────────────────────────────────────────────────────
function Badge({bg,color,children}){ return <span style={{fontSize:11,padding:"2px 8px",borderRadius:10,background:bg,color,whiteSpace:"nowrap"}}>{children}</span>; }
function ScoreCircle({score,size}){
  const sz=size||44; const c=scoreColor(score);
...
function Toast({toast}){ if(!toast) return null; return <div style={{position:"fixed",top:16,left:"50%",transform:"translateX(-50%)",zIndex:9999,background:toast.ok===false?"#7f1d1d":"#14532d",color:"#fff",padding:"8px 18px",borderRadius:20,fontSize:13,fontWeight:500,boxShadow:"0 2px 8px rgba(0,0,0,.25)",whiteSpace:"nowrap",pointerEvents:"none"}}>{toast.msg}</div>; }

// ── ItemCard — memo + stable callbacks via useRef trick ────────────────
// Uses a resultsRef so the textarea never loses focus on re-render
const ItemCard = memo(function ItemCard({itemId,itemLabel,itemCat,zoneIcon,zonelabel,showZone,isCustom,statut,comment,action,deadlineType,deadline,photoUrls,isExpanded,onStatut,onField,onToggle,onRemove,onAddPhoto,onRemovePhoto,fileRef}){
  ...
  return(
    <div style={{border:"0.5px solid "+(statut==="NS"?"#fca5a5":statut==="M"?"#fcd34d":"var(--color-border-tertiary)"),borderRadius:10,marginBottom:8,background:statut?"var(--color-background-primary)":"var(--color-background-secondary)"}}>
      <div style={{padding:"10px 12px"}}>
        <div style={{display:"flex",alignItems:"flex-start",gap:8,marginBottom:8}}>
          <div style={{flex:1,minWidth:0}}>
            {showZone&&<div style={{fontSize:10,color:"#ED7D31",marginBottom:1}}>{zoneIcon} {zonelabel}</div>}
            <div style={{fontSize:10,color:"var(--color-text-secondary)",marginBottom:2}}>{itemCat}</div>
            <div style={{fontSize:13,color:"var(--color-text-primary)",lineHeight:1.4}}>{itemLabel}</div>
          </div>
        ...
        <div style={{display:"flex",gap:5}}>
          {STATUTS.map(s=>(
            <button key={s.val} onClick={()=>onStatut(itemId,s.val)}
              style={{flex:1,padding:"6px 2px",fontSize:11,borderRadius:6,fontWeight:statut===s.val?500:400,border:"1.5px solid "+(statut===s.val?s.color:s.color+"55"),background:statut===s.val?s.bg:"transparent",color:statut===s.val?s.color:"var(--color-text-secondary)",cursor:"pointer"}}>
              {s.short} {s.val}
            </button>
          ))}
          <button onClick={()=>onToggle(itemId)} style={{padding:"6px 10px",fontSize:13,borderRadius:6,border:"0.5px solid var(--color-border-tertiary)",background:"none",cursor:"pointer",color:"var(--color-text-secondary)"}}>
            {isExpanded?"▲":"▼"}
          </button>
        </div>
      </div>
      {isExpanded&&(
        <div style={{padding:"0 12px 12px",borderTop:"0.5px solid var(--color-border-tertiary)",paddingTop:10}}>
          <div style={{marginBottom:10}}>
            <label style={{fontSize:12,color:"var(--color-text-secondary)",display:"block",marginBottom:3}}>Commentaire</label>
            <textarea value={comment||""} onChange={e=>onField(itemId,"comment",e.target.value)} placeholder="Observations..." rows={2}
              style={{width:"100%",boxSizing:"border-box",fontSize:12,borderRadius:6,border:"0.5px solid var(--color-border-tertiary)",padding:"6px 8px",resize:"vertical",background:"var(--color-background-primary)",color:"var(--color-text-primary)"}}/>
          </div>
          <div style={{marginBottom:10}}>
            <label style={{fontSize:12,color:"var(--color-text-secondary)",display:"block",marginBottom:3}}>Plan d'action</label>
            <textarea value={action||""} onChange={e=>onField(itemId,"action",e.target.value)} placeholder="Action corrective..." rows={2}
              style={{width:"100%",boxSizing:"border-box",fontSize:12,borderRadius:6,border:"0.5px solid var(--color-border-tertiary)",padding:"6px 8px",resize:"vertical",background:"var(--color-background-primary)",color:"var(--color-text-primary)"}}/>
          </div>
          <div style={{marginBottom:10}}>
            <label style={{fontSize:12,color:"var(--color-text-secondary)",display:"block",marginBottom:6}}>Délai de correction</label>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:6,marginBottom:sel==="date"?8:0}}>
              {DEADLINE_TYPES.map(dt=>(
                <button key={dt.val} onClick={()=>onField(itemId,"deadlineType",dt.val)}
                  style={{padding:"8px 4px",borderRadius:8,border:"1.5px solid "+(sel===dt.val?dt.color:dt.color+"44"),background:sel===dt.val?dt.bg:"transparent",color:sel===dt.val?dt.color:"var(--color-text-secondary)",cursor:"pointer",textAlign:"center"}}>
          ...
          <div>
            <label style={{fontSize:12,color:"var(--color-text-secondary)",display:"block",marginBottom:6}}>Photos</label>
            <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
              {(photoUrls||[]).map((ph,i)=>(
                <div key={i} style={{position:"relative",width:60,height:60,borderRadius:6,overflow:"hidden",border:"0.5px solid var(--color-border-tertiary)"}}>
              ...
              <button onClick={()=>fileRef.current&&fileRef.current.click()} style={{width:60,height:60,borderRadius:6,border:"1.5px dashed var(--color-border-secondary)",background:"none",cursor:"pointer",fontSize:24,color:"var(--color-text-secondary)"}}>+</button>
...
export default function App(){
  ...
  // customItems keyed by zoneId: { [zoneId]: [{id, cat, label}] }
  const [customItems, setCustomItems] = useState({});
  const [showAddItemZone, setShowAddItemZone] = useState(null); // zone id where add form is open
  const [newItemLabel,setNewItemLabel]= useState("");
  const [newItemCat,  setNewItemCat]  = useState("Hygiène & qualité");
  const fileRefs = useRef({});
  const autoRef  = useRef(null);

  const ZONES_LIVE = ZONES.map(z=>({
    ...z,
    items: [...z.items, ...((customItems[z.id])||[])]
  }));
  const ALL_CATS = [...new Set(ZONES_LIVE.flatMap(z=>z.items.map(i=>i.cat)))].sort();

  useEffect(()=>{
    Promise.all([dbLoad(),draftsLoad(),magasinsLoad()]).then(([data,dl,mags])=>{
      setAudits(data||[]); setDrafts(dl||[]); setMagasins(mags||[]); setPage("home");
    });
  },[]);
  ...
  useEffect(()=>{
    if(page==="audit"&&headerDone&&currentDraftId){
      autoRef.current=setInterval(async()=>{
        ...
        await draftsSave(up); setDrafts(up);
      },30000);
    }
  ...
  const onRemoveCustom = useCallback((id)=>{
    setCustomItems(prev=>{
      ...
      Object.keys(prev).forEach(zid=>{
        next[zid]=prev[zid].filter(i=>i.id!==id);
      });
      return next;
    });
  ...
  const allActions=[];
  audits.forEach(a=>{
    const az=ZONES.map(z=>({...z,items:[...z.items,...((a.customItems||{})[z.id]||[])]}));
  ...
  function startNewAudit(){ setResults({}); setZoneIdx(0); setHeaderDone(false); setAuditMode("zone"); setSelectedCat(null); setCurrentDraftId(null); setCustomItems({}); setShowAddItemZone(null); setNewItemLabel(""); setHeader({date:today(),heure:"",magasin:"",superviseur:"",responsable:""}); setPage("audit"); }
  ...
  async function saveDraftManual(){
    ...
    if(currentDraftId){
      ...
      const up=[...all,nd]; await draftsSave(up); setDrafts(up); setCurrentDraftId(id);
    }
  ...
  async function discardDraft(id){ await draftDelete(id); setDrafts(d=>d.filter(x=>x.id!==id)); showToast("Brouillon supprimé",false); }

  async function saveAudit(){
    const audit={id:Date.now().toString(),...header,results,score:scoreOf(results),counts:{...counts},customItems};
    const up=[audit,...audits]; setAudits(up); await dbSave(up);
    if(currentDraftId){ await draftDelete(currentDraftId); setDrafts(d=>d.filter(x=>x.id!==currentDraftId)); setCurrentDraftId(null); }
    const nm=header.magasin.trim();
    if(nm&&!magasins.includes(nm)){ const um=[...magasins,nm]; setMagasins(um); await magasinsSave(um); }
    showToast("Audit sauvegardé"); setPage("summary");
  }

  async function resolveAction(auditId,itemId){
    const up=audits.map(a=>a.id!==auditId?a:{...a,results:{...a.results,[itemId]:{...(a.results[itemId]||{}),resolved:true}}});
    setAudits(up); await dbSave(up); showToast("Action résolue");
  }

  async function deleteAudit(id){
    if(!window.confirm("Supprimer cet audit ?")) return;
    const up=audits.filter(a=>a.id!==id); setAudits(up); await dbSave(up); setViewAudit(null); showToast("Supprimé",false);
  }
  ...
  if(reportAudit){
    ...
    const auditZones=ZONES.map(z=>({...z,items:[...z.items,...((audit.customItems||{})[z.id]||[])]}));
    ...
    return(
      <div style={{maxWidth:620,margin:"0 auto",padding:"1rem"}}>
        ...
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12,flexWrap:"wrap",gap:8}}>
          <button onClick={()=>setReportAudit(null)} style={{background:"none",border:"none",cursor:"pointer",fontSize:14,color:"var(--color-text-secondary)"}}>← Retour</button>
          <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
            <button onClick={()=>{const el=document.getElementById("rapport-word");if(!el)return;const r=document.createRange();r.selectNode(el);window.getSelection().removeAllRanges();window.getSelection().addRange(r);document.execCommand("copy");window.getSelection().removeAllRanges();showToast("Rapport copié — colle dans Word / Google Docs");}} style={{padding:"8px 14px",borderRadius:8,border:"none",background:"#ED7D31",color:"#fff",cursor:"pointer",fontWeight:500,fontSize:13}}>📋 Copier pour Word</button>
            <button onClick={()=>copyText(mailBody)} style={{padding:"8px 14px",borderRadius:8,border:"0.5px solid #ED7D31",background:"none",color:"#ED7D31",cursor:"pointer",fontWeight:500,fontSize:13}}>📧 Corps mail</button>
          </div>
        </div>
        <div style={{background:"#f0fdf4",border:"0.5px solid #86efac",borderRadius:8,padding:"10px 14px",fontSize:12,color:"#166534",lineHeight:1.7,marginBottom:14}}>
          <strong>PDF :</strong> Copier → Coller dans Google Docs → Fichier → Télécharger → PDF
        </div>
        ...
        <div style={{background:"var(--color-background-secondary)",borderRadius:10,padding:"12px 14px",marginTop:14}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8}}>
            <div style={{fontSize:13,fontWeight:500,color:"var(--color-text-primary)"}}>📧 Corps du mail</div>
            <button onClick={()=>copyText(mailBody)} style={{fontSize:12,padding:"5px 12px",borderRadius:8,border:"0.5px solid #ED7D31",background:"none",color:"#ED7D31",cursor:"pointer",fontWeight:500}}>📋 Copier</button>
          </div>
          <pre style={{fontSize:11,color:"var(--color-text-secondary)",whiteSpace:"pre-wrap",fontFamily:"monospace",lineHeight:1.5,maxHeight:180,overflowY:"auto",margin:0,userSelect:"all",WebkitUserSelect:"all"}}>{mailBody}</pre>
        </div>
        <button onClick={()=>downloadCSV(toCSVSingle(audit),"Audit_"+(audit.magasin||"").replace(/\s+/g,"_")+"_"+audit.date+".csv")} style={{width:"100%",padding:"10px",borderRadius:8,border:"0.5px solid #ED7D31",background:"none",color:"#ED7D31",cursor:"pointer",fontSize:13,marginTop:10}}>📊 Télécharger CSV</button>
      </div>
    );
  }

  // ── LOADING ────────────────────────────────────────────────────────────
  if(page==="loading") return <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"4rem",gap:12}}><div style={{fontSize:36}}>🛒</div><div style={{fontSize:14,color:"var(--color-text-secondary)"}}>Chargement...</div></div>;

  // ── HOME ───────────────────────────────────────────────────────────────
  if(page==="home") return(
    <div style={{maxWidth:480,margin:"0 auto",padding:"1.5rem 1rem"}}>
      <Toast toast={toast}/>
      <div style={{textAlign:"center",marginBottom:20}}><div style={{fontSize:38}}>🛒</div><div style={{fontSize:19,fontWeight:500,color:"var(--color-text-primary)"}}>Audit Superviseur</div><div style={{fontSize:12,color:"var(--color-text-secondary)"}}>Réseau Mandarine — 2026</div></div>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:10,marginBottom:16}}>
        ...
        {drafts.length>0&&(<div style={{border:"1.5px solid #ED7D31",borderRadius:10,overflow:"hidden"}}><div style={{background:"#fff7ed",padding:"8px 14px",display:"flex",alignItems:"center",gap:8}}><span style={{fontSize:16}}>📝</span><div style={{fontSize:13,fontWeight:500,color:"#9a3412"}}>Brouillons en cours ({drafts.length})</div></div>{drafts.map(d=>(<div key={d.id} style={{display:"flex",alignItems:"center",gap:8,padding:"9px 12px",borderTop:"0.5px solid #fed7aa",background:"var(--color-background-primary)"}}><div style={{flex:1,minWidth:0}}><div style={{fontSize:13,fontWeight:500,color:"var(--color-text-primary)",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{(d.header&&d.header.magasin)||"Magasin non défini"}</div><div style={{fontSize:11,color:"var(--color-text-secondary)"}}>{(d.header&&d.header.date)||""} — {(d.header&&d.header.superviseur)||"?"}</div></div><button onClick={()=>loadDraft(d)} style={{fontSize:12,padding:"5px 10px",borderRadius:8,border:"none",background:"#ED7D31",color:"#fff",cursor:"pointer"}}>Reprendre</button><button onClick={()=>discardDraft(d.id)} style={{fontSize:12,padding:"5px 8px",borderRadius:8,border:"0.5px solid #fca5a5",background:"none",color:"#dc2626",cursor:"pointer"}}>✕</button></div>))}</div>)}
        <button onClick={()=>setPage("actions")} style={{padding:"14px 16px",borderRadius:10,border:"0.5px solid var(--color-border-tertiary)",background:"var(--color-background-secondary)",fontSize:14,cursor:"pointer",textAlign:"left",display:"flex",alignItems:"center",gap:10,color:"var(--color-text-primary)"}}><span style={{fontSize:22}}>🔧</span><span style={{flex:1}}>Actions correctives</span>{allActions.length>0&&<Badge bg={urgentCount>0?"#fee2e2":"#fef3c7"} color={urgentCount>0?"#dc2626":"#d97706"}>{allActions.length} en cours</Badge>}</button>
        <button onClick={()=>{ setDbPage("list"); setViewAudit(null); setPage("history"); }} style={{padding:"14px 16px",borderRadius:10,border:"0.5px solid var(--color-border-tertiary)",background:"var(--color-background-secondary)",fontSize:14,cursor:"pointer",textAlign:"left",display:"flex",alignItems:"center",gap:10,color:"var(--color-text-primary)"}}><span style={{fontSize:22}}>📋</span><span style={{flex:1}}>Historique</span><span style={{fontSize:12,color:"var(--color-text-secondary)"}}>{audits.length} audit{audits.length>1?"s":""}</span></button>
      ...
      {audits.slice(0,4).map(a=>(<div key={a.id} onClick={()=>{ setViewAudit(a); setDbPage("detail"); setPage("history"); }} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 12px",borderRadius:8,border:"0.5px solid var(--color-border-tertiary)",background:"var(--color-background-primary)",marginBottom:6,cursor:"pointer"}}><ScoreCircle score={a.score||0}/><div style={{flex:1,minWidth:0}}><div style={{fontSize:13,fontWeight:500,color:"var(--color-text-primary)",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{a.magasin}</div><div style={{fontSize:12,color:"var(--color-text-secondary)"}}>{a.date} — {a.superviseur}</div></div><div style={{display:"flex",gap:4}}>{(a.counts&&a.counts.NS&&a.counts.NS>0)?<Badge bg="#fee2e2" color="#dc2626">{a.counts.NS} NS</Badge>:null}{(a.counts&&a.counts.M&&a.counts.M>0)?<Badge bg="#fef3c7" color="#d97706">{a.counts.M} M</Badge>:null}</div><span style={{color:"var(--color-text-secondary)",fontSize:16}}>›</span></div>))}
    </div>
  );
  ...
  if(page==="audit"&&!headerDone) return(
    <div style={{maxWidth:480,margin:"0 auto",padding:"1.5rem 1rem"}}>
      <Toast toast={toast}/>
      <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:16}}><button onClick={()=>setPage("home")} style={{background:"none",border:"none",cursor:"pointer",fontSize:18,padding:0,color:"var(--color-text-secondary)"}}>←</button><div style={{fontSize:17,fontWeight:500,color:"var(--color-text-primary)"}}>Nouvelle visite</div></div>
      <div style={{background:"var(--color-background-secondary)",borderRadius:12,padding:"1rem",marginBottom:16}}>
        {[{l:"Date",k:"date",t:"date"},{l:"Heure",k:"heure",t:"time"},{l:"Superviseur",k:"superviseur",t:"text",p:"Nom du superviseur"},{l:"Responsable magasin",k:"responsable",t:"text",p:"Nom du gérant"}].map(f=>(<div key={f.k} style={{marginBottom:12}}><label style={{fontSize:12,color:"var(--color-text-secondary)",display:"block",marginBottom:3}}>{f.l}</label><input type={f.t} value={header[f.k]||""} placeholder={f.p||""} onChange={e=>setHeader(h=>({...h,[f.k]:e.target.value}))} style={{width:"100%",boxSizing:"border-box"}}/></div>))}
        <div style={{marginBottom:12}}><label style={{fontSize:12,color:"var(--color-text-secondary)",display:"block",marginBottom:3}}>Magasin</label><input type="text" value={header.magasin||""} placeholder="Nom du magasin" list="mag-list" onChange={e=>setHeader(h=>({...h,magasin:e.target.value}))} style={{width:"100%",boxSizing:"border-box"}}/><datalist id="mag-list">{magasins.map(m=><option key={m} value={m}/>)}</datalist></div>
        <div><label style={{fontSize:12,color:"var(--color-text-secondary)",display:"block",marginBottom:6}}>Mode d'audit</label><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>{[{v:"zone",icon:"🗺️",l:"Par zone",d:"Zone par zone"},{v:"cat",icon:"🏷️",l:"Par catégorie",d:"Hygiène, Sécurité…"}].map(m=>(<button key={m.v} onClick={()=>setAuditMode(m.v)} style={{padding:"10px 8px",borderRadius:8,border:"1.5px solid "+(auditMode===m.v?"#ED7D31":"var(--color-border-tertiary)"),background:auditMode===m.v?"#fff7ed":"transparent",color:auditMode===m.v?"#ED7D31":"var(--color-text-secondary)",cursor:"pointer",textAlign:"center"}}><div style={{fontSize:18}}>{m.icon}</div><div style={{fontSize:12,fontWeight:auditMode===m.v?500:400}}>{m.l}</div><div style={{fontSize:10,opacity:.8}}>{m.d}</div></button>))}</div></div>
      </div>
  ...
  if(page==="audit"&&headerDone){
    ...
    return(
      <div style={{maxWidth:560,margin:"0 auto",padding:"1rem"}}>
        <Toast toast={toast}/>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8}}>
          <div style={{display:"flex",alignItems:"center",gap:8}}><button onClick={()=>setPage("home")} style={{background:"none",border:"none",cursor:"pointer",fontSize:16,padding:0,color:"var(--color-text-secondary)"}}>←</button><div><div style={{fontSize:13,fontWeight:500,color:"var(--color-text-primary)"}}>{header.magasin}</div><div style={{fontSize:11,color:"var(--color-text-secondary)"}}>{header.superviseur} — {header.date}</div></div></div>
          <div style={{display:"flex",alignItems:"center",gap:6}}><span style={{fontSize:12,color:"var(--color-text-secondary)"}}>{pct}%</span><button onClick={saveDraftManual} style={{fontSize:12,padding:"5px 10px",borderRadius:20,border:"1px solid #ED7D31",background:"none",color:"#ED7D31",cursor:"pointer"}}>Sauv.</button><button onClick={saveAudit} style={{fontSize:12,padding:"5px 12px",borderRadius:20,border:"none",background:"#ED7D31",color:"#fff",cursor:"pointer",fontWeight:500}}>Terminer</button></div>
        </div>
        <div style={{height:4,background:"var(--color-background-secondary)",borderRadius:2,marginBottom:10,overflow:"hidden"}}><div style={{height:"100%",width:pct+"%",background:"#ED7D31",transition:"width .3s"}}/></div>
        <div style={{display:"flex",gap:6,marginBottom:10}}>
          <button onClick={()=>{setAuditMode("zone");setSelectedCat(null);}} style={{flex:1,padding:"7px",borderRadius:8,border:"1.5px solid "+(auditMode==="zone"?"#ED7D31":"var(--color-border-tertiary)"),background:auditMode==="zone"?"#ED7D31":"transparent",color:auditMode==="zone"?"#fff":"var(--color-text-secondary)",cursor:"pointer",fontSize:12,fontWeight:auditMode==="zone"?500:400}}>🗺️ Par zone</button>
          <button onClick={()=>{setAuditMode("cat");if(!selectedCat)setSelectedCat(ALL_CATS[0]);}} style={{flex:1,padding:"7px",borderRadius:8,border:"1.5px solid "+(auditMode==="cat"?"#ED7D31":"var(--color-border-tertiary)"),background:auditMode==="cat"?"#ED7D31":"transparent",color:auditMode==="cat"?"#fff":"var(--color-text-secondary)",cursor:"pointer",fontSize:12,fontWeight:auditMode==="cat"?500:400}}>🏷️ Par catégorie</button>
        </div>
        ...
        {auditMode==="zone"&&(<>
          <div style={{display:"flex",gap:5,overflowX:"auto",paddingBottom:6,marginBottom:10,WebkitOverflowScrolling:"touch"}}>
            {ZONES_LIVE.map((z,i)=>{
              ...
              return(<button key={z.id} onClick={()=>setZoneIdx(i)} style={{flexShrink:0,padding:"5px 10px",borderRadius:20,fontSize:12,fontWeight:zoneIdx===i?500:400,border:"1.5px solid "+(zoneIdx===i?"#ED7D31":"var(--color-border-tertiary)"),background:zoneIdx===i?"#ED7D31":"var(--color-background-primary)",color:zoneIdx===i?"#fff":hasNS?"#dc2626":allDone?"#16a34a":"var(--color-text-secondary)",cursor:"pointer"}}>{z.icon} {z.label.split(" ")[0]}{done>0&&<span style={{marginLeft:3,fontSize:10,opacity:.8}}>{done}/{z.items.length}</span>}</button>);
            })}
          </div>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8}}><div style={{fontSize:15,fontWeight:500,color:"var(--color-text-primary)"}}>{zone.icon} {zone.label}</div><span style={{fontSize:12,color:"var(--color-text-secondary)"}}>{zone.items.filter(i=>results[i.id]&&results[i.id].statut).length}/{zone.items.length}</span></div>
          {zone.items.map(item=>renderItem(item,false,(customItems[zone.id]||[]).some(c=>c.id===item.id)))}

          {/* Bouton ajout item — toutes les zones */}
          <div style={{marginTop:8}}>
            {showAddItemZone!==zone.id?(
              ...
              <div style={{background:"var(--color-background-secondary)",borderRadius:10,padding:"12px",border:"1px solid #ED7D31"}}>
                <div style={{fontSize:13,fontWeight:500,color:"var(--color-text-primary)",marginBottom:10}}>Nouvel item — {zone.label}</div>
                <div style={{marginBottom:8}}>
                  <label style={{fontSize:12,color:"var(--color-text-secondary)",display:"block",marginBottom:3}}>Catégorie</label>
                ...
                <div style={{display:"flex",gap:8}}>
                  ...
                  <button onClick={()=>{ setShowAddItemZone(null); setNewItemLabel(""); }} style={{flex:1,padding:"9px",borderRadius:8,border:"0.5px solid var(--color-border-tertiary)",background:"none",color:"var(--color-text-secondary)",cursor:"pointer"}}>Annuler</button>
                </div>
              </div>
            )}
          </div>
          <div style={{display:"flex",gap:8,marginTop:10}}>
            <button disabled={zoneIdx===0} onClick={()=>setZoneIdx(i=>Math.max(0,i-1))} style={{flex:1,padding:"10px",borderRadius:8,border:"0.5px solid var(--color-border-tertiary)",background:"none",cursor:zoneIdx===0?"not-allowed":"pointer",color:"var(--color-text-secondary)",opacity:zoneIdx===0?0.4:1}}>Précédent</button>
        ...
        {auditMode==="cat"&&(<>
          <div style={{display:"flex",gap:5,overflowX:"auto",paddingBottom:6,marginBottom:10,WebkitOverflowScrolling:"touch"}}>
            {ALL_CATS.map(cat=>{
              ...
              return(<button key={cat} onClick={()=>setSelectedCat(cat)} style={{flexShrink:0,padding:"5px 10px",borderRadius:20,fontSize:12,fontWeight:selectedCat===cat?500:400,border:"1.5px solid "+(selectedCat===cat?"#ED7D31":"var(--color-border-tertiary)"),background:selectedCat===cat?"#ED7D31":"var(--color-background-primary)",color:selectedCat===cat?"#fff":hasNS?"#dc2626":allDone?"#16a34a":"var(--color-text-secondary)",cursor:"pointer"}}>{CAT_ICONS[cat]||"📋"} {cat.split(" ")[0]}{done>0&&<span style={{marginLeft:3,fontSize:10,opacity:.8}}>{done}/{items.length}</span>}</button>);
            })}
          </div>
          {selectedCat&&(()=>{
            // group catItems by zone for add-item support
            const zonesByCat = ZONES_LIVE.filter(z=>z.items.some(i=>i.cat===selectedCat));
            return(<>
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8}}>
                <div style={{fontSize:15,fontWeight:500,color:"var(--color-text-primary)"}}>{CAT_ICONS[selectedCat]||"📋"} {selectedCat}</div>
                <span style={{fontSize:12,color:"var(--color-text-secondary)"}}>{catItems.filter(i=>results[i.id]&&results[i.id].statut).length}/{catItems.length}</span>
              </div>
              {catItems.map(item=>renderItem(item,true,(customItems[item.zoneid]||[]).some(c=>c.id===item.id)))}
              {/* Add item per zone in this category */}
              {zonesByCat.map(z=>(
                <div key={z.id} style={{marginTop:8}}>
                  {showAddItemZone!==("cat_"+z.id)?(
                    ...
                    <div style={{background:"var(--color-background-secondary)",borderRadius:10,padding:"12px",border:"1px solid #ED7D31"}}>
                      <div style={{fontSize:13,fontWeight:500,color:"var(--color-text-primary)",marginBottom:10}}>Nouvel item — {z.label} / {selectedCat}</div>
                      <div style={{marginBottom:10}}>
                        <label style={{fontSize:12,color:"var(--color-text-secondary)",display:"block",marginBottom:3}}>Description</label>
                        <input type="text" value={newItemLabel} onChange={e=>setNewItemLabel(e.target.value)}
  ...
  if(page==="summary"){
    ...
    return(<div style={{maxWidth:480,margin:"0 auto",padding:"2rem 1rem"}}>
      <Toast toast={toast}/>
      <div style={{textAlign:"center",marginBottom:20}}><div style={{fontSize:52,fontWeight:500,color:scoreColor(sc),lineHeight:1}}>{sc}%</div><div style={{fontSize:14,color:"var(--color-text-secondary)",marginTop:4}}>Score de conformité</div><div style={{fontSize:12,color:"#16a34a",marginTop:2}}>Audit sauvegardé</div></div>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:8}}>
        ...
        <button onClick={()=>setPage("actions")} style={{padding:"11px",borderRadius:8,border:"0.5px solid var(--color-border-tertiary)",background:"var(--color-background-secondary)",color:"var(--color-text-primary)",cursor:"pointer",fontSize:14}}>🔧 Actions correctives</button>
        <button onClick={()=>setPage("home")} style={{padding:"11px",borderRadius:8,border:"none",background:"none",color:"var(--color-text-secondary)",cursor:"pointer",fontSize:13}}>← Retour</button>
      </div>
    </div>);
  }
  ...
  if(page==="history"){
    if(dbPage==="db"){
      ...
      return(<div style={{maxWidth:640,margin:"0 auto",padding:"1.5rem 1rem"}}>
        <Toast toast={toast}/>
        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:14}}><button onClick={()=>setPage("home")} style={{background:"none",border:"none",cursor:"pointer",fontSize:18,padding:0,color:"var(--color-text-secondary)"}}>←</button><div style={{fontSize:17,fontWeight:500,color:"var(--color-text-primary)"}}>Base de données consolidée</div></div>
        <div style={{display:"flex",gap:6,overflowX:"auto",paddingBottom:6,marginBottom:14,WebkitOverflowScrolling:"touch"}}>{["tous",...magasinsInDB].map(m=>(<button key={m} onClick={()=>setFilterMag(m)} style={{flexShrink:0,padding:"5px 12px",borderRadius:20,fontSize:12,border:"1.5px solid "+(filterMag===m?"#ED7D31":"var(--color-border-tertiary)"),background:filterMag===m?"#ED7D31":"var(--color-background-primary)",color:filterMag===m?"#fff":"var(--color-text-secondary)",cursor:"pointer",fontWeight:filterMag===m?500:400}}>{m==="tous"?"Tous":m}</button>))}</div>
        {listeFiltre.length>0&&(()=>{const scores=listeFiltre.map(a=>scoreOf(a.results));const avg=Math.round(scores.reduce((a,b)=>a+b,0)/scores.length);return(<div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8,marginBottom:14}}>{[{l:"Score moyen",v:avg+"%",c:scoreColor(avg)},{l:"Meilleur",v:Math.max(...scores)+"%",c:scoreColor(Math.max(...scores))},{l:"Plus bas",v:Math.min(...scores)+"%",c:scoreColor(Math.min(...scores))}].map(s=>(<div key={s.l} style={{background:"var(--color-background-secondary)",borderRadius:8,padding:"10px 8px",textAlign:"center"}}><div style={{fontSize:20,fontWeight:500,color:s.c}}>{s.v}</div><div style={{fontSize:11,color:"var(--color-text-secondary)"}}>{s.l}</div></div>))}</div>);})()}
        {listeFiltre.length===0?<div style={{textAlign:"center",padding:"2rem",color:"var(--color-text-secondary)"}}>Aucun audit pour ce magasin</div>:(<div style={{overflowX:"auto",marginBottom:14}}><table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}><thead><tr style={{background:"#ED7D31",color:"#fff"}}>{["Magasin","Date","Superviseur","Score","S","M","NS","NA"].map(h=><th key={h} style={{padding:"8px 10px",textAlign:"left",whiteSpace:"nowrap"}}>{h}</th>)}</tr></thead><tbody>{listeFiltre.map((a,i)=>{const sc=scoreOf(a.results);const c=a.counts||{};return(<tr key={a.id} onClick={()=>{setViewAudit(a);setDbPage("detail");}} style={{background:i%2===0?"var(--color-background-primary)":"var(--color-background-secondary)",cursor:"pointer"}}><td style={{padding:"7px 10px",maxWidth:140,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{a.magasin}</td><td style={{padding:"7px 10px",whiteSpace:"nowrap"}}>{a.date}</td><td style={{padding:"7px 10px",whiteSpace:"nowrap"}}>{a.superviseur}</td><td style={{padding:"7px 10px",fontWeight:500,color:scoreColor(sc)}}>{sc}%</td><td style={{padding:"7px 10px",color:"#16a34a"}}>{c.S||0}</td><td style={{padding:"7px 10px",color:"#d97706"}}>{c.M||0}</td><td style={{padding:"7px 10px",color:"#dc2626"}}>{c.NS||0}</td><td style={{padding:"7px 10px",color:"#6b7280"}}>{c.NA||0}</td></tr>);})}</tbody></table></div>)}
        <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:10}}>
          <button onClick={()=>downloadCSV(toCSVConsolide(audits,null),"Synthese_Mandarine_"+today()+".csv")} style={{flex:1,minWidth:140,padding:"10px",borderRadius:8,border:"none",background:"#ED7D31",color:"#fff",cursor:"pointer",fontSize:13,fontWeight:500}}>Synthèse tous</button>
          <button onClick={()=>downloadCSV(toCSVDetail(audits,null),"Detail_Mandarine_"+today()+".csv")} style={{flex:1,minWidth:140,padding:"10px",borderRadius:8,border:"0.5px solid #ED7D31",background:"none",color:"#ED7D31",cursor:"pointer",fontSize:13,fontWeight:500}}>Détail tous</button>
        ...
        <div style={{padding:"10px 14px",background:"var(--color-background-secondary)",borderRadius:8,fontSize:12,color:"var(--color-text-secondary)"}}><strong style={{color:"var(--color-text-primary)"}}>Excel :</strong> Fichier → Ouvrir → .csv → UTF-8 → séparateur virgule.</div>
      </div>);
    }
    if(dbPage==="detail"&&viewAudit){
      const sc=scoreOf(viewAudit.results); const c=viewAudit.counts||{};
      const vz=ZONES.map(z=>z.id==="custom"?{...z,items:viewAudit.customItems||[]}:z);
      const issues=[];
      ...
      return(<div style={{maxWidth:540,margin:"0 auto",padding:"1.5rem 1rem"}}>
        <Toast toast={toast}/>
        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:14}}><button onClick={()=>setDbPage("list")} style={{background:"none",border:"none",cursor:"pointer",fontSize:18,padding:0,color:"var(--color-text-secondary)"}}>←</button><div style={{fontSize:17,fontWeight:500,color:"var(--color-text-primary)"}}>Détail de l'audit</div></div>
        <div style={{background:"var(--color-background-secondary)",borderRadius:10,padding:"12px 14px",marginBottom:14}}><div style={{display:"flex",alignItems:"center",gap:12}}><ScoreCircle score={sc} size={48}/><div><div style={{fontSize:14,fontWeight:500,color:"var(--color-text-primary)"}}>{viewAudit.magasin}</div><div style={{fontSize:12,color:"var(--color-text-secondary)"}}>{viewAudit.date} {viewAudit.heure||""} — {viewAudit.superviseur}</div><div style={{fontSize:12,color:"var(--color-text-secondary)"}}>Gérant : {viewAudit.responsable}</div></div></div></div>
        ...
        {issues.length>0&&(<><div style={{fontSize:14,fontWeight:500,marginBottom:8,color:"var(--color-text-primary)"}}>Points non conformes ({issues.length})</div>{issues.map((iss,i)=>{const st2=STATUTS.find(s=>s.val===iss.r.statut);const dl=getDeadlineBadge(iss.r);const resolved=iss.r.resolved;return(<div key={i} style={{border:"0.5px solid var(--color-border-tertiary)",borderRadius:8,padding:"10px 12px",marginBottom:6,background:resolved?"#f0fdf4":"var(--color-background-primary)"}}><div style={{display:"flex",gap:6,flexWrap:"wrap",alignItems:"center",marginBottom:4}}>{st2&&<Badge bg={st2.bg} color={st2.color}>{st2.label}</Badge>}{resolved&&<Badge bg="#dcfce7" color="#16a34a">Résolue</Badge>}{dl&&<Badge bg={dl.bg} color={dl.color}>{dl.text}</Badge>}</div><div style={{fontSize:12,color:"var(--color-text-secondary)",marginBottom:2}}>{iss.icon} {iss.zone}</div><div style={{fontSize:13,color:"var(--color-text-primary)"}}>{iss.item}</div>{iss.r.action&&<div style={{fontSize:12,color:"#d97706",marginTop:4}}>Action : {iss.r.action}</div>}</div>);})}</>)}
    ...
    return(<div style={{maxWidth:480,margin:"0 auto",padding:"1.5rem 1rem"}}>
      <Toast toast={toast}/>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14}}><div style={{display:"flex",alignItems:"center",gap:10}}><button onClick={()=>setPage("home")} style={{background:"none",border:"none",cursor:"pointer",fontSize:18,padding:0,color:"var(--color-text-secondary)"}}>←</button><div style={{fontSize:17,fontWeight:500,color:"var(--color-text-primary)"}}>Historique</div></div><div style={{display:"flex",gap:8,alignItems:"center"}}><span style={{fontSize:12,color:"var(--color-text-secondary)"}}>{audits.length} audit{audits.length>1?"s":""}</span>{audits.length>0&&<button onClick={()=>setDbPage("db")} style={{fontSize:12,padding:"5px 10px",borderRadius:8,border:"0.5px solid #ED7D31",background:"none",color:"#ED7D31",cursor:"pointer"}}>Base DB</button>}</div></div>
      {audits.length===0&&<div style={{textAlign:"center",padding:"3rem 1rem",color:"var(--color-text-secondary)"}}><div style={{fontSize:36,marginBottom:8}}>📋</div><div>Aucun audit enregistré</div></div>}
      {audits.map(a=>(<div key={a.id} style={{display:"flex",alignItems:"center",gap:10,padding:"12px 14px",borderRadius:10,border:"0.5px solid var(--color-border-tertiary)",background:"var(--color-background-primary)",marginBottom:8}}>
        <div onClick={()=>{ setViewAudit(a); setDbPage("detail"); }} style={{display:"flex",alignItems:"center",gap:10,flex:1,minWidth:0,cursor:"pointer"}}>
          ...
          <div style={{flex:1,minWidth:0}}>
            <div style={{fontSize:13,fontWeight:500,color:"var(--color-text-primary)",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{a.magasin}</div>
            <div style={{fontSize:12,color:"var(--color-text-secondary)"}}>{a.date} — {a.superviseur}</div>
          </div>
          <div style={{display:"flex",gap:4}}>
            {(a.counts&&a.counts.NS&&a.counts.NS>0)?<Badge bg="#fee2e2" color="#dc2626">{a.counts.NS} NS</Badge>:null}
            {(a.counts&&a.counts.M&&a.counts.M>0)?<Badge bg="#fef3c7" color="#d97706">{a.counts.M} M</Badge>:null}
          </div>
  ...
  if(page==="actions"){
    ...
    function ACard({act}){
      ...
      return(<div style={{border:"0.5px solid "+(act.deadlineType==="immediat"||(dl&&dl.urgent)?"#fca5a5":act.deadlineType==="continu"?"#c4b5fd":"var(--color-border-tertiary)"),borderRadius:10,padding:"10px 12px",marginBottom:8,background:"var(--color-background-primary)"}}>
        <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:8,marginBottom:4}}><div style={{flex:1,minWidth:0}}><div style={{fontSize:11,color:"var(--color-text-secondary)",marginBottom:2}}>{act.zoneIcon} {act.zone} · {act.cat}</div><div style={{fontSize:13,color:"var(--color-text-primary)",lineHeight:1.4}}>{act.item}</div></div>{st&&<Badge bg={st.bg} color={st.color}>{st.label}</Badge>}</div>
        <div style={{fontSize:11,color:"var(--color-text-secondary)",marginBottom:4}}>{act.magasin} — {act.auditDate}</div>
        {act.comment&&<div style={{fontSize:12,color:"var(--color-text-secondary)",marginBottom:2}}>{act.comment}</div>}
    ...
    return(<div style={{maxWidth:560,margin:"0 auto",padding:"1.5rem 1rem"}}>
      <Toast toast={toast}/>
      <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:14}}><button onClick={()=>setPage("home")} style={{background:"none",border:"none",cursor:"pointer",fontSize:18,padding:0,color:"var(--color-text-secondary)"}}>←</button><div style={{fontSize:17,fontWeight:500,color:"var(--color-text-primary)"}}>Actions correctives</div>{allActions.length>0&&<Badge bg="#fef3c7" color="#d97706">{allActions.length} en cours</Badge>}</div>
      {allActions.length===0&&<div style={{textAlign:"center",padding:"3rem 1rem"}}><div style={{fontSize:40,marginBottom:8}}>✅</div><div style={{fontSize:15,fontWeight:500,color:"var(--color-text-primary)"}}>Aucune action en cours</div></div>}
      ...
      <Sec title="Sans deadline" bg="var(--color-background-secondary)" color="var(--color-text-secondary)" items={noDate} icon="⏳"/>
    </div>);
  }
